...
|---|---|---|---|
| `canTask` (`src/can_vesc.cpp`) | `TASK_PRIO_CAN` = 6 | 4096 | `twai_receive()`, keep VESC STATUS 1..6 frames, decode, smooth (EMA), write `g_state.vesc`; every `VESC_POLL_MS` send one `COMM_GET_VALUES_SELECTIVE` request, reassemble and CRC-check the reply into `g_state.vesc_ext`, latch the last fault; bus-off recovery and health counters |
...
| `loop()` supervisor (`src/main.cpp`) | 1 | Arduino default | sole Task-WDT subscriber (`WDT_TIMEOUT_MS`), fed only while the CAN, GNSS and display heartbeats are fresh (the trip task has none); 1 Hz VESC and GNSS log lines, 5 s TRIP line (`LOG_TRIP_MS`), 10 s SYS line with the display, button and poll counters; LED |

Data flow:

```
 VESC ---CAN---> transceiver -> TWAI ISR -> RX queue (64 frames) -> canTask --+
      <--------- 1 request frame / s (single shot) <--------------- canTask   +--> g_state (one mutex) --> displayTask -> U8g2 fonts -> DIYables_OLED_SSD1309 -> I2C -> OLED
...
| `include/shared_state.h`, `src/shared_state.cpp` | `g_state` (`vesc`, `vesc_ext`, `can`, `gnss`, `trip`, `disp`), mutex, snapshots, heartbeats, `fresh()` |
...
Any 128x64 SSD1309 module works (Waveshare, DIYables, LCDWIKI, Diymore / ACEIRMC 2.42" or 1.54", Adafruit 2719); the firmware drives it with the DIYables_OLED_SSD1309 library (Adafruit_SSD1306-compatible API on Adafruit GFX) and draws text with U8g2 fonts through U8g2_for_Adafruit_GFX. Most 7-pin modules ship in 4-wire SPI mode and must be switched to I2C first.
...
`CAN_LISTEN_ONLY 0` (default) runs the TWAI controller in NORMAL mode, so it generates the ACK bit for every frame it receives. That ACK is required when the VESC and this board are the only two nodes: the VESC's STM32 bxCAN has no retransmit limit, so with a non-acknowledging listener it goes error-passive and repeats the same stale STATUS frame forever ("lonely VESC"). The poll request is sent single-shot for the mirror-image reason: with the VESC powered off it is simply lost (`VESC poll: N request(s) not acknowledged on the bus (VESC off? TEC=N)`, error-passive after ~16 polls, never bus-off) instead of being retransmitted for ever. Set `-DCAN_LISTEN_ONLY=1 -DVESC_POLL_MS=0` (no ACK, TX electrically silent) only when another node acknowledges anyway: a second VESC, a BMS, a permanently attached VESC Tool CAN adapter.
...
128 x 64, white on black, `OLED_ROTATION 0`. Text is drawn with U8g2 fonts through the U8g2_for_Adafruit_GFX library on top of the DIYables/Adafruit GFX frame buffer: `u8g2_font_logisoso24_tn` for the speed (24 px digits, tabular 15 px advance; the font has digits, `-`, `.` and `:` only), `u8g2_font_6x10_tf` for all other text (6 px monospace, 7 px caps, has the degree sign) and `u8g2_font_6x13B_tf` (bold, same 6 px advance) for the title bars. Only the three referenced fonts are linked (~4.7 KB of flash); the rest of the U8g2 font file costs nothing. A row of small text is 21 characters wide.

Seven screens, cycled by the button (section "Button" below); the detail screens carry their page number `n/7` in the title bar. All value fields are fixed-width cells; `--` means stale or never received.
...
```
[  1512][I][main.cpp:..] setup(): boat-motor 0.1.0 (built Sep  6 2026 19:35:02) reset=POWERON
[  1513][I][main.cpp:..] logConfig(): cfg CAN : tx=3 rx=2 500 kbit/s mode=NORMAL(ack+poll) rxq=64 vesc_id=-1 poles=14 poll=1000 ms own_id=120
...
[  1515][I][main.cpp:..] logConfig(): cfg GNSS: rx=4 tx=5 rate=200 ms unit=kn rxbuf=2048
[  1530][I][can_vesc.cpp:..] ...: CAN: driver installed, state RUNNING: mode=NORMAL (ACK, transmits only GET_VALUES_SELECTIVE polls) bitrate=500k tx=GPIO3 rx=GPIO2 rx_queue=64 tx_queue=4 sample_point=80% poll=1000 ms own_id=120 mask=0x003EC03C
[  1535][I][gnss_ubx.cpp:..] ...: GNSS: Serial1 (HP UART1) RX=GPIO4 TX=GPIO5 ring=2048 B, first baud 115200, target baud 115200, rate 200 ms
[  1537][I][trip.cpp:..] ...: trip: task started (period 200 ms, window 10 s, unit Wh/NM, min speed 500 mm/s)
[  1540][I][display_oled.cpp:..] ...: OLED: button on GPIO7 (active low, pull-up, debounce 30 ms, long press 1500 ms, auto-return 60000 ms)
[  1541][I][esp32-hal-i2c-ng.c:..] i2cInit(): Initializing I2C Master: num=0 sda=22 scl=23 freq=400000
[  1575][I][display_oled.cpp:..] ...: OLED: init ok (SSD1309 128x64, I2C 0x3C @ 400000 Hz, sda=22 scl=23 rst=14)
[  1610][I][display_oled.cpp:..] ...: OLED: task started (poll 20 ms, frame 250 ms, 7 screens, button GPIO7, auto-return 60000 ms, demo=0)
[  1611][I][main.cpp:..] setup(): started: can=1 gnss=1 display=1 trip=1
[  1650][I][can_vesc.cpp:..] ...: VESC: locked onto controller id 74
[  5000][I][can_vesc.cpp:..] ...: VESC id=74 frames=212 other=0 | erpm=0 rpm=0 duty=0.000 Im=0.0A(0.0) | Iin=0.2A(0.2) Vin=50.1V(50.1) P=10W | Tfet=41.0C Tmot=n/a | ... | age s1=12ms s2=340ms s3=341ms s4=15ms s5=9ms s6=never | ext: fault=NONE last=none Tmos=40.5/41.0/40.0C Iavg=0.00/0.20A Id/Iq=0.00/0.00A Vd/Vq=0.000/0.000V tachoAbs=0 st=- id=74 polls=3 ok=3 bad=0 to=0 age=410ms | can=RUN tec=0 rec=0 ...
[  6612][I][trip.cpp:..] ...: TRIP run=5s moving=0s dist=0.000NM wh=0.0 whc=0.0 now=-- avg=-- win=4s P=10W src=counters resets=0
[  8420][I][gnss_ubx.cpp:..] ...: GNSS: UBX detected at 9600 baud (MON-VER reply)
[  8630][I][gnss_ubx.cpp:..] ...: GNSS: PROTVER 18.00 -> legacy CFG configuration
[  8631][I][gnss_ubx.cpp:..] ...: GNSS: switching UART1 9600 -> 115200 baud (CFG-PRT)
[  8850][I][gnss_ubx.cpp:..] ...: GNSS: UART1 switched to 115200 baud (MON-VER reply)
[  8930][I][gnss_ubx.cpp:..] ...: GNSS: NMEA off: 8/8 ACKed
[  8940][I][gnss_ubx.cpp:..] ...: GNSS: CFG-RATE meas=200 nav=1: ACK
[  8950][I][gnss_ubx.cpp:..] ...: GNSS: CFG-MSG NAV-PVT on: ACK
[  8970][I][gnss_ubx.cpp:..] ...: GNSS: configured=1 velned=0 (baud 115200, PROTVER 18.00)
[  9001][I][gnss_ubx.cpp:..] ...: GNSS phase=RUN baud=115200 protver=18.00 cfg=1 fix=3 ok=1 sats=9 spd=0.00kn sAcc=0.40m/s ... age=110ms good=57 bad=0 redetect=0
[ 10000][I][main.cpp:..] loop(): SYS up=10s reset=POWERON heap=... minheap=... lockfail=0 | can=RUN tec=0 rec=0 busoff=0 | disp ok=1 refreshes=.. skipped=.. dim=0 scr=0 btn=0 | poll sent=9 ok=9 bad=0 to=0 | hb can=.. gnss=.. disp=..
```

An M9/M10 receiver logs `GNSS: PROTVER 34.10 -> VALSET configuration`, `GNSS: switching UART1 38400 -> 115200 baud (VALSET)` and four `GNSS: VALSET ...: ACK` lines (UART1OUTPROT, MSGOUT, RATE, NAVSPG-DYNMODEL) instead of the CFG-MSG/CFG-RATE ones; one that still runs at 115200 from an earlier boot (backup battery) is found within a second and skips the switch. The first `TRIP` line shows `now=-- avg=--` until the boat has covered `EFF_MIN_DIST_M`. Before a VESC is connected the VESC line prints `never` for every age, `can=RUN` with `frames=0` and `polls=0` (no id to poll yet), the cells show `--` and the CAN line `CAN idle`; that is expected. With `-DVESC_POLL_MS=0` the install line ends in `poll=off` and the VESC line carries `| poll=off` instead of the `| ext:` section. The LED blinks at 1 Hz while VESC data is fresh and slowly otherwise. `OLED: refresh N ms (#N, screen N, stack free min N B)` per frame and `VESC poll: reply 42 B from id 74 in N ms: ...` per reply need `CORE_DEBUG_LEVEL=4`.
...
| Repeated `CAN: BUS_OFF`, `CAN: error passive` or `CAN: N bus error alert(s)` with the VESC on | termination (60 Ohm across CANH/CANL), CANH/CANL swapped, grounds not commoned, RS not grounded, bitrate mismatch |
