...
| supervisor loop in `app_main()` (`src/main.cpp`) | 1 | 8192 (`CONFIG_ESP_MAIN_TASK_STACK_SIZE`) | sole Task-WDT subscriber (`WDT_TIMEOUT_MS`), fed only while the CAN, GNSS and display heartbeats are fresh (the trip task has none); 1 Hz VESC and GNSS log lines, 5 s TRIP line (`LOG_TRIP_MS`), 10 s SYS line with the display, button and poll counters; LED |
...
| `platformio.ini`, `boards/dfrobot_firebeetle2_esp32c6.json`, `sdkconfig.defaults`, `partitions.csv`, `CMakeLists.txt`, `src/CMakeLists.txt` | build: pinned platform and envs, board definition, ESP-IDF configuration, partition table, IDF project files |
...
I (1611) main: started: can=1 gnss=1 display=1 trip=1
...
I (10000) main: SYS up=10s reset=POWERON heap=... minheap=... lockfail=0 | can=RUN tec=0 rec=0 busoff=0 | disp ok=1 refreshes=.. skipped=.. dim=0 scr=0 btn=0 | poll sent=9 ok=9 bad=0 to=0 | hb can=.. gnss=.. disp=..
```
...
| A change in `sdkconfig.defaults` has no effect | the generated `sdkconfig.dfrobot_firebeetle2_esp32c6` (or `sdkconfig.demo`) already exists: delete it and rebuild |
